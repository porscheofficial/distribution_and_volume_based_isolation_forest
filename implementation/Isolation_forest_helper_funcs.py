...
class PatternSpaceIsolationForest:
    ...
    def train_IF(self, dataset, n_estimators=50, contamination=0.1,max_features=1.0):
    ...
    def generate_axis_aligned_hyper_rectangles_forest(self, model, dataset):
        ...
        for p in range(len(model.estimators_)):
            ...
            is_leaves, node_depth, children_left, children_right, _, thresholds = self.iterate_through_tree(model, tree_number=p)
    ...
    def populate_pattern_space_tree(self, ds1, model, tree_number, volumes_pattern_space_forest):

        print(f"tree: {tree_number} -> Number of axis aligned hyper rectangles {len(volumes_pattern_space_forest[tree_number])}")

        number_of_patterns_in_single_space = len(volumes_pattern_space_forest[tree_number])
    ...
    def populate_pattern_space_forest(self, ds1, model, volumes_pattern_space_forest):
        ...
        for tree, _ in enumerate(np.arange(0,number_of_estimators,1)):
            tree_patterns_occurences, filtered_sorted_thresholds = self.populate_pattern_space_tree(ds1, model, tree, volumes_pattern_space_forest)
    ...
    def debug_tree(self, model, dataset ,tree_number=0):
        ...
        is_leaves, node_depth, _, _, _, _ = self.iterate_through_tree(model, tree_number)
